...
namespace NN {
    namespace Layers {
        PoolMaxLayer::PoolMaxLayer(std::vector<int> dependencies,
        ...
        void PoolMaxLayer::backpropagate() {
            int output_size = output_width * output_height * output_depth;

            int block_size = (output_size + 511) / 512;
            NN::CUDA::backprop_pool_max_layer << <block_size, 512 >> > (input, input_gradient, output_gradient,
                input_width, input_height, input_depth,
                filter_width, filter_height, filter_depth,
                output_width, output_height, output_depth,
                output_size);
        ...
        int PoolMaxLayer::get_parameters_size() {
            return 0;
        }

        void PoolMaxLayer::update_dependencies(vector<NN::Layers::Layer*> layer_dependencies) {
            input = layer_dependencies[0]->get_output_iterator();
            input_gradient = layer_dependencies[0]->get_output_gradient_iterator();
        }

        void PoolMaxLayer::save(NN::File& file) {
            int id = 7;
            file.save(id);
        ...
        void PoolMaxLayer::load(NN::File& file) {
        ...
        PoolMaxLayer::~PoolMaxLayer() = default;
    }
}